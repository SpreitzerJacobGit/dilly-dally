...
`[ACC-1]` Signing in with any credential listed above succeeds on the first attempt and lands on the dashboard.
...
`[SYS-15]` A headline number on the dashboard equals what the underlying listing shows when opened.
...
`[SEED-1]` One active trip exists: "Portland → Las Vegas" from Portland, OR to Las Vegas, NV, with one pending waypoint, Bishop, CA, biasing the route through California.
